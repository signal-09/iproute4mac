...
def teardown_module(module):
    ...
    if _VLAN:
        system(f"sudo ifconfig {_VLAN} destroy")


def test_ip_link_add_vlan(script_runner):
    res = script_runner.run(f"sudo ip link add link {_EN} type vlan id {_VLAN_ID}".split())
    globals()["_VLAN"] = res.stdout.rstrip()
    assert res.returncode == 0
    assert res.stdout.startswith("vlan")
    assert res.stderr == ""

    res = script_runner.run(f"sudo ip link add link {_EN} name {_VLAN} type vlan id {_VLAN_ID}".split())
...
def test_ip_addr_add(script_runner):
    res = script_runner.run(f"sudo ip addr add {_VLAN_IP}/24 brd + dev {_VLAN}".split())
    assert res.returncode == 0
    assert res.stdout == ""
    assert res.stderr == ""
...
def test_ip_link_del_vlan(script_runner):
    res = script_runner.run(f"sudo ip link del {_VLAN}".split())
    assert res.returncode == 0
    assert res.stdout == ""
    assert res.stderr == ""
