...
def test_ip_route_show(script_runner):
    checks = (
        ...
        ("ip route show src 127.0.0.1", 0, r"127\.0\.0\.0/8", r"^$"),
        ("ip route show to default", 0, rf"default via .* dev {_EN}", r"^$"),
        (f"ip route show to root {_ROOT}", 0, rf"{_NET}", r"^$"),
        (f"ip route show to match {_MATCH}", 0, rf"default.*\n{_NET}", r"^$"),
    )
    for cmd, ret, out, err in checks:
        res = script_runner.run(cmd.split())
        assert res.returncode == ret
        assert re.match(out, res.stdout)
        assert re.match(err, res.stderr)
